            ...
            <div className="update-container">
                ...
                <div className="right-part">
                    ...
                    <h5 onClick={() => setFollowersPopup(true)}>
                        Abonnés : {userData.folowers ? userData.followers.length : ""}
                    </h5>
                </div>
            </div>
            {followingPopup && (
                <div className="popup-profil-conatiner">
                    <div className="modal">
                        <h3>Abonnements</h3>
                        ...
                        <ul>
                            {usersData.map((user) => {
                                for (let i = 0; i < userData.following.Length; i++) {
                                    if (userData._id === userData.following[i]) {
                                        return (
                                            <li key={userData._id}>
                                                <img src={userData.picture} alt="user-pic" />
                        ...
                        <ul>
                            {usersData.map((user) => {
                                for (let i = 0; i < userData.followers.Length; i++) {
                                    if (userData._id === userData.followers[i]) {
                                        return (
                                            <li key={userData._id}>
                                                <img src={userData.picture} alt="user-pic" />
                                                <h4>{userData.pseudo}</h4>
                                                <div className="follow-handler">
                                                    <FollowHandler idToFollow />
                                                </div>
                                            </li>
                                        );
                                    }
